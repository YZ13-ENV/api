...
export const bum = {
      ...
      try {
        ...
        const url =
          order && category
            ...
            : order
              ? `${api_host}/shots/all/${order}?onlyDrafts=true`
  ...
  shots: {
    search: async (
      ...
      try {
        if (order === "following" && !uid) throw Error("uid is not provided");
        const headers = new Headers();
        const authHeader = authorizationHeader();
        headers.append("authorization", authHeader || "");
        const url =
      ...
      try {
        ...
        const url =
          order && category
            ...
            : order
              ? `${api_host}/shots/all/${order}${uid ? `?uid=${uid}` : ""}`
              : `${api_host}/shots/all/popular${uid ? `?uid=${uid}` : ""}`;
        const res = await fetch(url, { method: "GET", headers: headers });
        if (res.ok) return (await res.json()) as ChunkResponse<DocShotData[]>;
        return { count: 0, data: [], next: "" };
      } catch (e) {
